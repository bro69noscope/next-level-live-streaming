...
async function loadStreamerbotInstances() {
    ...
    for (const envKey of envKeys) {
        const { value: port, error } = await getPortConfigValue(REPO_ROOT, `streamerbot.${envKey}.streamerbot_ws.port`);
        if (error || port === null) {
            console.error(`StreamFeedApp: failed to load port for ${envKey}: ${error}`);
            continue;
        }
        instances.push({ name: envKey, host: "127.0.0.1", port: port });
    }
    return instances;
}
